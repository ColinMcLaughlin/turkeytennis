...
`).join(""),C=()=>{const e={};return r.teams.forEach((t,n)=>{e[n]={wins:0,losses:0}}),r.matchSchedule.forEach(t=>{if(t.score!=="-"){const n=t.score.split("-").map(a=>parseInt(a.trim()));n.length===2&&n[0]!==n[1]&&(n[0]>n[1]?(e[t.team1Idx].wins++,e[t.team2Idx].losses++):n[1]>n[0]&&(e[t.team2Idx].wins++,e[t.team1Idx].losses++))}}),e},N=()=>r.matchSchedule?`
  ...
  `},I=async e=>{if(!y)return console.error("Firestore not initialized.");try{const t=S(y,$);await x(t,e,{merge:!0}),console.log("Tournament state saved successfully.")}catch(t){console.error("Error saving state:",t);const n=document.getElementById("result");n&&(n.textContent="Error: Could not save score to database.",n.className="result error")}},te=async()=>{if(!y)return;const e=S(y,$);(await z(e)).exists()||(console.log("No existing tournament data found. Initializing new tournament."),await x(e,{teams:P,matchSchedule:L,bracketMatches:M,bracketPlayActive:!1,alternates:w}))},R=()=>{const e=document.getElementById("app");if(!(!e||!r.teams))try{e.innerHTML=`
      <div>

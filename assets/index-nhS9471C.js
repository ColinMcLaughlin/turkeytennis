...
`).join(""),X=()=>E.map(e=>`<li>${e}</li>`).join(""),K=()=>n.teams.map((e,t)=>`
...
`).join(""),F=()=>{const e={};return n.teams.forEach((t,o)=>{e[o]={wins:0,losses:0}}),n.matchSchedule.forEach(t=>{if(t.score!=="-"){const o=t.score.split("-").map(a=>parseInt(a.trim()));o.length===2&&o[0]!==o[1]&&(o[0]>o[1]?(e[t.team1Idx].wins++,e[t.team2Idx].losses++):o[1]>o[0]&&(e[t.team2Idx].wins++,e[t.team1Idx].losses++))}}),e},V=()=>n.matchSchedule?`
  ...
  `},B=async e=>{if(!g)return console.error("Firestore not initialized.");try{const t=$(g,w);await x(t,e,{merge:!0}),console.log("Tournament state saved successfully.")}catch(t){console.error("Error saving state:",t);const o=document.getElementById("result");o&&(o.textContent="Error: Could not save score to database.",o.className="result error")}},te=async()=>{if(!g)return;const e=$(g,w);(await _(e)).exists()||(console.log("No existing tournament data found. Initializing new tournament."),await x(e,{teams:L,matchSchedule:M,bracketMatches:C,bracketPlayActive:!1,alternates:E}))},N=()=>{const e=document.getElementById("app");if(!(!e||!n.teams))try{e.innerHTML=`
      <div>

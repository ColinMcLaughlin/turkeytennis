...
`).join(""),Y=()=>T.map(e=>`<li>${e}</li>`).join(""),G=()=>i.teams.map((e,t)=>`
...
`).join(""),C=()=>{const e={};return i.teams.forEach((t,n)=>{e[n]={wins:0,losses:0}}),i.matchSchedule.forEach(t=>{if(t.score!=="-"){const n=t.score.split("-").map(a=>parseInt(a.trim()));n.length===2&&n[0]!==n[1]&&(n[0]>n[1]?(e[t.team1Idx].wins++,e[t.team2Idx].losses++):n[1]>n[0]&&(e[t.team2Idx].wins++,e[t.team1Idx].losses++))}}),e},K=()=>i.matchSchedule?`
  ...
  `},B=async e=>{if(!y)return console.error("Firestore not initialized.");try{const t=S(y,$);await P(t,e,{merge:!0}),console.log("Tournament state saved successfully.")}catch(t){console.error("Error saving state:",t);const n=document.getElementById("result");n&&(n.textContent="Error: Could not save score to database.",n.className="result error")}},ee=async()=>{if(!y)return;const e=S(y,$);(await J(e)).exists()||(console.log("No existing tournament data found. Initializing new tournament."),await P(e,{teams:x,matchSchedule:L,bracketMatches:M,bracketPlayActive:!1,alternates:T}))},N=()=>{const e=document.getElementById("app");!e||!i.teams||(e.innerHTML=`
    <div>
  ...
  `,se(),i.bracketPlayActive&&ae())},A=e=>{document.querySelectorAll(".tab-button").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".tab-pane").forEach(a=>a.classList.remove("active"));const t=document.querySelector(`[data-tab="${e}"]`),n=document.getElementById(e);t&&n&&(t.classList.add("active"),n.classList.add("active"))},te=e=>{document.querySelectorAll(".subtab-button").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".subtab-pane").forEach(a=>a.classList.remove("active"));const t=document.querySelector(`[data-subtab="${e}"]`),n=document.getElementById(e);t&&n&&(t.classList.add("active"),n.classList.add("active"))},ae=()=>{const e=document.getElementById("bracket-match-select");if(!e)return;e.innerHTML='<option value="">Choose a bracket match...</option>';const t=i.bracketMatches;[{round:"qf",matches:t.quarterfinals,prefix:"Quarterfinal"},{round:"sf",matches:t.semifinals,prefix:"Semifinal"},{round:"f",matches:t.finals,prefix:"Finals"},{round:"csf",matches:t.consolationSemis,prefix:"Consolation Semifinal"},{round:"cf",matches:t.consolationFinals,prefix:"Consolation Finals"}].forEach(({round:a,matches:s,prefix:b})=>{s.forEach((o,r)=>{const d=o.team1.includes("Pool")||o.team1.includes("Winner")||o.team1.includes("Loser")||o.team1.includes("TBD");if(o.score==="-"&&!d){let l=`${b} ${a!=="f"&&a!=="cf"?r+1:""}`.trim();e.innerHTML+=`<option value="${a}-${r}">${l}: ${o.team1} vs ${o.team2}</option>`}})})},se=()=>{const e=document.getElementById("app");if(!e)return;e.addEventListener("click",o=>{o.target.classList.contains("tab-button")&&o.target.getAttribute("data-tab")?A(o.target.getAttribute("data-tab")):o.target.classList.contains("subtab-button")&&o.target.getAttribute("data-subtab")?te(o.target.getAttribute("data-subtab")):o.target.classList.contains("team-card")&&o.target.getAttribute("data-team-id")&&A("teams")});const t=document.getElementById("passwordBtn");t&&(t.onclick=()=>{const o=document.getElementById("password").value,r=document.getElementById("password-result");o===U?(k=!0,N(),r.textContent="Unlocked!",r.className="result success"):(r.textContent="Incorrect password",r.className="result error")});const n=document.getElementById("submitBtn");n&&(n.onclick=async()=>{if(!k)return;const o=parseInt(document.getElementById("match-select").value),r=parseInt(document.getElementById("score1").value),d=parseInt(document.getElementById("score2").value),l=document.getElementById("result");if(isNaN(o)||o<0||isNaN(r)||isNaN(d)){l.textContent="Please select a match and enter valid scores",l.className="result error";return}if(i.matchSchedule[o].score!=="-"){l.textContent="Score already submitted for this match.",l.className="result error";return}const c=i.matchSchedule[o];let p,m=[...i.matchSchedule];if(r===d){m[o].score=`${r} - ${d}`,await B({matchSchedule:m}),l.textContent=`${c.team1} ${r} - ${d} ${c.team2} (Tied)`,l.className="result tie";return}p=r>d?c.team1:c.team2,m[o].score=`${r} - ${d}`,await B({matchSchedule:m}),l.textContent=`${c.team1} ${r} - ${d} ${c.team2} | Winner: ${p}`,l.className="result success",document.getElementById("match-select").value="",document.getElementById("score1").value="",document.getElementById("score2").value=""});const a=document.getElementById("concludeBtn");a&&(a.onclick=async()=>{const o=C(),r=i.teams.filter(c=>c.pool==="A").map(c=>{const p=i.teams.indexOf(c);return{...c,teamIdx:p,record:o[p]}}).sort((c,p)=>p.record.wins-c.record.wins),d=i.teams.filter(c=>c.pool==="B").map(c=>{const p=i.teams.indexOf(c);return{...c,teamIdx:p,record:o[p]}}).sort((c,p)=>p.record.wins-c.record.wins);let l=JSON.parse(JSON.stringify(i.bracketMatches));l.quarterfinals[0].team1=r[0].name,l.quarterfinals[0].team2=d[3].name,l.quarterfinals[1].team1=d[0].name,l.quarterfinals[1].team2=r[3].name,l.quarterfinals[2].team1=r[1].name,l.quarterfinals[2].team2=d[2].name,l.quarterfinals[3].team1=d[1].name,l.quarterfinals[3].team2=r[2].name,await B({bracketMatches:l,bracketPlayActive:!0}),document.getElementById("result").textContent="Pool play concluded! Bracket play has begun.",document.getElementById("result").className="result success"});const s=document.getElementById("submitBracketBtn");s&&(s.onclick=async()=>{const o=document.getElementById("bracket-match-select").value,r=parseInt(document.getElementById("bracket-score1").value),d=parseInt(document.getElementById("bracket-score2").value),l=document.getElementById("result");if(!o||isNaN(r)||isNaN(d)||r===d){l.textContent="Please select a match and enter valid (non-tied) scores",l.className="result error";return}const[c,p]=o.split("-"),m=parseInt(p);let u=JSON.parse(JSON.stringify(i.bracketMatches)),f,I,w;const g=(h,v,F)=>{f=h[v],I=r>d?f.team1:f.team2,w=r>d?f.team2:f.team1,f.score=`${r} - ${d}`,f.winner=I,F(I,w)};c==="qf"?g(u.quarterfinals,m,(h,v)=>{m===0?(u.semifinals[0].team1=h,u.consolationSemis[0].team1=v):m===1?(u.semifinals[0].team2=h,u.consolationSemis[0].team2=v):m===2?(u.semifinals[1].team1=h,u.consolationSemis[1].team1=v):m===3&&(u.semifinals[1].team2=h,u.consolationSemis[1].team2=v)}):c==="sf"?g(u.semifinals,m,h=>{m===0?u.finals[0].team1=h:m===1&&(u.finals[0].team2=h)}):c==="f"?g(u.finals,m,()=>{}):c==="csf"?g(u.consolationSemis,m,h=>{m===0?u.consolationFinals[0].team1=h:m===1&&(u.consolationFinals[0].team2=h)}):c==="cf"&&g(u.consolationFinals,m,()=>{}),await B({bracketMatches:u}),l.textContent=`Score submitted: ${f.team1} ${r} - ${d} ${f.team2} | Winner: ${I}`,l.className="result success",document.getElementById("bracket-match-select").value="",document.getElementById("bracket-score1").value="",document.getElementById("bracket-score2").value=""});const b=document.getElementById("resetBtn");b&&(b.onclick=async()=>{await B({teams:x,matchSchedule:L,bracketMatches:M,bracketPlayActive:!1,alternates:T}),document.getElementById("result").textContent="Tournament has been reset!",document.getElementById("result").className="result error"})},oe=async()=>{try{const e=D(Q);y=R(e),O("error");const t=_(e);E?await q(t,E):await W(t),console.log("Firebase authenticated successfully."),await ee();const n=S(y,$);H(n,a=>{a.exists()?(i=a.data(),document.getElementById("loading").style.display="none",document.getElementById("app").style.display="block",N()):console.warn("Tournament state document does not exist. A new one should be initialized shortly.")})}catch(e){console.error("Firebase Initialization Failed:",e),document.getElementById("loading").innerHTML=`

...
`).join(""),C=()=>{const e={};return i.teams.forEach((t,o)=>{e[o]={wins:0,losses:0}}),i.matchSchedule.forEach(t=>{if(t.score!=="-"){const o=t.score.split("-").map(a=>parseInt(a.trim()));o.length===2&&o[0]!==o[1]&&(o[0]>o[1]?(e[t.team1Idx].wins++,e[t.team2Idx].losses++):o[1]>o[0]&&(e[t.team2Idx].wins++,e[t.team1Idx].losses++))}}),e},K=()=>i.matchSchedule?`
  ...
  `},I=async e=>{if(!y)return console.error("Firestore not initialized.");try{const t=$(y,w);await P(t,e,{merge:!0}),console.log("Tournament state saved successfully.")}catch(t){console.error("Error saving state:",t);const o=document.getElementById("result");o&&(o.textContent="Error: Could not save score to database.",o.className="result error")}},ee=async()=>{if(!y)return;const e=$(y,w);(await J(e)).exists()||(console.log("No existing tournament data found. Initializing new tournament."),await P(e,{teams:x,matchSchedule:L,bracketMatches:M,bracketPlayActive:!1,alternates:E}))},F=()=>{const e=document.getElementById("app");if(!(!e||!i.teams))try{e.innerHTML=`
      <div>

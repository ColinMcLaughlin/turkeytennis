...
`).join(""),K=()=>E.map(e=>`<li>${e}</li>`).join(""),V=()=>n.teams.map((e,t)=>`
...
`).join(""),F=()=>{const e={};return n.teams.forEach((t,o)=>{e[o]={wins:0,losses:0}}),n.matchSchedule.forEach(t=>{if(t.score!=="-"){const o=t.score.split("-").map(a=>parseInt(a.trim()));o.length===2&&o[0]!==o[1]&&(o[0]>o[1]?(e[t.team1Idx].wins++,e[t.team2Idx].losses++):o[1]>o[0]&&(e[t.team2Idx].wins++,e[t.team1Idx].losses++))}}),e},Z=()=>n.matchSchedule?`
  ...
  `},te=()=>{if(!n.bracketMatches)return"<div>Loading brackets...</div>";const e=s=>`
  ...
  `},B=async e=>{if(!g)return console.error("Firestore not initialized.");try{const t=$(g,w);await L(t,e,{merge:!0}),console.log("Tournament state saved successfully.")}catch(t){console.error("Error saving state:",t);const o=document.getElementById("result");o&&(o.textContent="Error: Could not save score to database.",o.className="result error")}},ae=async()=>{if(!g)return;const e=$(g,w);(await U(e)).exists()||(console.log("No existing tournament data found. Initializing new tournament."),await L(e,{teams:M,matchSchedule:C,bracketMatches:N,bracketPlayActive:!1,alternates:E}))},R=()=>{const e=document.getElementById("app");if(!(!e||!n.teams))try{e.innerHTML=`
      <div>
    ...
    `,oe(),T("home"),D("pool-play"),n.bracketPlayActive&&se(),document.getElementById("loading").style.display="none",document.getElementById("app").style.display="block"}catch(t){console.error("Critical Rendering Error:",t),document.getElementById("loading").style.display="none",document.getElementById("app").style.display="block",e.innerHTML='<div style="color: red; padding: 2rem;">CRITICAL RENDERING ERROR: Failed to build the page content. Check the console (F12) for JavaScript errors.</div>'}},T=e=>{document.querySelectorAll(".tab-button").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".tab-pane").forEach(a=>a.classList.remove("active"));const t=document.querySelector(`[data-tab="${e}"]`),o=document.getElementById(e);t&&o&&(t.classList.add("active"),o.classList.add("active"))},D=e=>{document.querySelectorAll(".subtab-button").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".subtab-pane").forEach(a=>a.classList.remove("active"));const t=document.querySelector(`[data-subtab="${e}"]`),o=document.getElementById(e);t&&o&&(t.classList.add("active"),o.classList.add("active"))},se=()=>{const e=document.getElementById("bracket-match-select");if(!e)return;e.innerHTML='<option value="">Choose a bracket match...</option>';const t=n.bracketMatches;[{round:"qf",matches:t.quarterfinals,prefix:"Quarterfinal"},{round:"sf",matches:t.semifinals,prefix:"Semifinal"},{round:"f",matches:t.finals,prefix:"Finals"},{round:"csf",matches:t.consolationSemis,prefix:"Consolation Semifinal"},{round:"cf",matches:t.consolationFinals,prefix:"Consolation Finals"}].forEach(({round:a,matches:s,prefix:m})=>{s.forEach((p,r)=>{const i=p.team1.includes("Pool")||p.team1.includes("Winner")||p.team1.includes("Loser")||p.team1.includes("TBD");if(p.score==="-"&&!i){let d=`${m} ${a!=="f"&&a!=="cf"?r+1:""}`.trim();e.innerHTML+=`<option value="${a}-${r}">${d}: ${p.team1} vs ${p.team2}</option>`}})})},oe=()=>{const e=document.getElementById("app");if(!e)return;const t=r=>{if(r.target.classList.contains("tab-button")&&r.target.getAttribute("data-tab"))T(r.target.getAttribute("data-tab"));else if(r.target.classList.contains("subtab-button")&&r.target.getAttribute("data-subtab")){const i=r.target.getAttribute("data-subtab");D(i)}else r.target.classList.contains("team-card")&&r.target.getAttribute("data-team-id")&&T("teams")};e.removeEventListener("click",e.eventHandler),e.addEventListener("click",t),e.eventHandler=t;const o=document.getElementById("passwordBtn");o&&(o.onclick=()=>{const r=document.getElementById("password").value,i=document.getElementById("password-result");r===X?(S=!0,R(),i.textContent="Unlocked!",i.className="result success"):(i.textContent="Incorrect password",i.className="result error")});const a=document.getElementById("submitBtn");a&&(a.onclick=async()=>{if(!S)return;const r=parseInt(document.getElementById("match-select").value),i=parseInt(document.getElementById("score1").value),d=parseInt(document.getElementById("score2").value),l=document.getElementById("result");if(isNaN(r)||r<0||isNaN(i)||isNaN(d)){l.textContent="Please select a match and enter valid scores",l.className="result error";return}if(n.matchSchedule[r].score!=="-"){l.textContent="Score already submitted for this match.",l.className="result error";return}const c=n.matchSchedule[r];let h,u=[...n.matchSchedule];if(i===d){u[r].score=`${i} - ${d}`,await B({matchSchedule:u}),l.textContent=`${c.team1} ${i} - ${d} ${c.team2} (Tied)`,l.className="result tie";return}h=i>d?c.team1:c.team2,u[r].score=`${i} - ${d}`,await B({matchSchedule:u}),l.textContent=`${c.team1} ${i} - ${d} ${c.team2} | Winner: ${h}`,l.className="result success",document.getElementById("match-select").value="",document.getElementById("score1").value="",document.getElementById("score2").value=""});const s=document.getElementById("concludeBtn");s&&(s.onclick=async()=>{const r=F(),i=n.teams.filter(c=>c.pool==="A").map(c=>{const h=n.teams.indexOf(c);return{...c,teamIdx:h,record:r[h]}}).sort((c,h)=>h.record.wins-c.record.wins),d=n.teams.filter(c=>c.pool==="B").map(c=>{const h=n.teams.indexOf(c);return{...c,teamIdx:h,record:r[h]}}).sort((c,h)=>h.record.wins-c.record.wins);let l=JSON.parse(JSON.stringify(n.bracketMatches));l.quarterfinals[0].team1=i[0].name,l.quarterfinals[0].team2=d[3].name,l.quarterfinals[1].team1=d[0].name,l.quarterfinals[1].team2=i[3].name,l.quarterfinals[2].team1=i[1].name,l.quarterfinals[2].team2=d[2].name,l.quarterfinals[3].team1=d[1].name,l.quarterfinals[3].team2=i[2].name,await B({bracketMatches:l,bracketPlayActive:!0}),document.getElementById("result").textContent="Pool play concluded! Bracket play has begun.",document.getElementById("result").className="result success"});const m=document.getElementById("submitBracketBtn");m&&(m.onclick=async()=>{const r=document.getElementById("bracket-match-select").value,i=parseInt(document.getElementById("bracket-score1").value),d=parseInt(document.getElementById("bracket-score2").value),l=document.getElementById("result");if(!r||isNaN(i)||isNaN(d)||i===d){l.textContent="Please select a match and enter valid (non-tied) scores",l.className="result error";return}const[c,h]=r.split("-"),u=parseInt(h);let b=JSON.parse(JSON.stringify(n.bracketMatches)),f,k,A;const I=(v,y,_)=>{f=v[y],k=i>d?f.team1:f.team2,A=i>d?f.team2:f.team1,f.score=`${i} - ${d}`,f.winner=k,_(k,A)};c==="qf"?I(b.quarterfinals,u,(v,y)=>{u===0?(b.semifinals[0].team1=v,b.consolationSemis[0].team1=y):u===1?(b.semifinals[0].team2=v,b.consolationSemis[0].team2=y):u===2?(b.semifinals[1].team1=v,b.consolationSemis[1].team1=y):u===3&&(b.semifinals[1].team2=v,b.consolationSemis[1].team2=y)}):c==="sf"?I(b.semifinals,u,v=>{u===0?b.finals[0].team1=v:u===1&&(b.finals[0].team2=v)}):c==="f"?I(b.finals,u,()=>{}):c==="csf"?I(b.consolationSemis,u,v=>{u===0?b.consolationFinals[0].team1=v:u===1&&(b.consolationFinals[0].team2=v)}):c==="cf"&&I(b.consolationFinals,u,()=>{}),await B({bracketMatches:b}),l.textContent=`Score submitted: ${f.team1} ${i} - ${d} ${f.team2} | Winner: ${k}`,l.className="result success",document.getElementById("bracket-match-select").value="",document.getElementById("bracket-score1").value="",document.getElementById("bracket-score2").value=""});const p=document.getElementById("resetBtn");p&&(p.onclick=async()=>{await B({teams:M,matchSchedule:C,bracketMatches:N,bracketPlayActive:!1,alternates:E}),document.getElementById("result").textContent="Tournament has been reset!",document.getElementById("result").className="result error"})},ne=async()=>{const e=document.getElementById("loading"),t=document.getElementById("app");if(Object.keys(P).length===0){e&&(e.innerHTML='<div style="color: red; padding: 2rem; font-weight: bold;">FATAL ERROR: Firebase Configuration is Missing. Cannot connect to database.</div>');return}try{const o=q(P);g=J(o),j("error");const a=O(o);x?await W(a,x):await H(a),console.log("Firebase authenticated successfully."),await ae();const s=$(g,w);Q(s,m=>{m.exists()?(n=m.data(),R()):console.warn("Tournament state document does not exist. A new one should be initialized shortly.")})}catch(o){console.error("Firebase Initialization Failed:",o),e&&(e.innerHTML=`
        ...
        `),t&&(t.style.display="none"),e&&(e.style.display="block")}};document.addEventListener("DOMContentLoaded",ne);
